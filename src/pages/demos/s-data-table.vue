...
import type { RowSelection, ColumnConfig, SortInfo, ExpandableConfig, ScrollConfig } from '@/components/shared'
    ...
    return () => (
      <div class="p-4 bg-muted/30 rounded-lg border space-y-4">
        ...
        <div class="flex items-center gap-4 pt-2 border-t">
          ...
          <span class="text-xs text-muted-foreground">（{keepExpanded.value ? '缓存开启，数字不变' : '缓存关闭，每次展开数字变化'}）</span>
        </div>
      </div>
    )
  },
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground mt-1.5">灵活的数据表格组件，支持多种配置方式</p>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6">
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center gap-2 pt-1">
                <Badge variant="outline" class="font-normal"> 已选: {{ selectedRowKeys.length }} 项 </Badge>
                <Button variant="ghost" size="sm" :disabled="selectedRowKeys.length === 0" @click="clearSelection"> 清空 </Button>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div v-if="clickedRow" class="p-3 bg-muted/50 rounded-lg border text-sm space-y-1">
              ...
              <div v-else class="p-3 bg-muted/30 rounded-lg border border-dashed text-sm text-muted-foreground text-center">点击表格行查看效果</div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center gap-3">
                ...
                <SSelect v-model="paginationPosition" :options="paginationPositionOptions" :disabled="!enablePagination" placeholder="选择位置" />
              </div>
              ...
              <div v-if="enablePagination && !remoteMode" class="flex items-center gap-2 pt-1">
                <Badge variant="outline" class="font-normal"> 第 {{ paginationPage }} / {{ Math.ceil(data.length / paginationPageSize) }} 页 </Badge>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div v-if="currentSort" class="flex items-center gap-2 pt-1">
                ...
                <Button variant="ghost" size="sm" @click="currentSort = undefined"> 清除 </Button>
              </div>
              <div v-else class="text-xs text-muted-foreground">点击表头开始排序</div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center gap-2 pt-1">
                <Badge variant="outline" class="font-normal"> 已展开: {{ expandedRowKeys.length }} 项 </Badge>
                <Button variant="ghost" size="sm" :disabled="expandedRowKeys.length === 0" @click="clearExpanded"> 收起全部 </Button>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <template #cell-status="{ value }">
            <Badge
              ...
              :class="value === 'active' ? 'bg-primary' : 'text-destructive border-destructive/50'"
            >
              {{ value === 'active' ? '正常' : '禁用' }}
            </Badge>
          </template>
    ...
    <Card>
      ...
      <CardContent>
        <pre class="text-xs bg-muted/50 p-4 rounded-lg overflow-x-auto border"><code>&lt;DataTable
  :data="data"
  :columns="columns"
  size="{{ size }}"
  :show-header="{{ showHeader }}"
