...
<template>
  <div class="space-y-6">
    ...
    <!-- 配置面板 -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg border">
      <div class="space-y-4">
        <h3 class="font-semibold text-sm">基础配置</h3>

        <div class="flex items-center gap-2">
          <label class="text-sm w-20">表格大小</label>
          <select v-model="size" class="flex-1 px-2 py-1.5 border rounded text-sm bg-background">
      ...
      <div class="space-y-4">
        ...
        <div class="text-sm">
          已选择: <span class="font-medium">{{ selectedRowKeys.length }}</span> 项
        </div>
    ...
    >
      ...
      <template #empty>
        <div class="flex flex-col items-center gap-3 py-8">
          ...
          <button class="px-3 py-1.5 text-sm bg-primary text-primary-foreground rounded hover:bg-primary/90">
            添加用户
          </button>
        </div>
      </template>
    ...
    <div class="p-4 bg-muted/30 rounded-lg border">
      ...
      <pre class="text-xs bg-background p-3 rounded overflow-x-auto"><code>&lt;DataTable
  :data="data"
  :columns="columns"
  size="{{ size }}"
  :show-header="{{ showHeader }}"
