...
function handlePageSizeChange(value: AcceptableValue) {
  ...
  const newPage = Math.min(props.page, Math.ceil(props.total / newPageSize) || 1)
...
<template>
  <div :class="cn('flex flex-wrap items-center gap-4', props.class)">
    <!-- 总条数 -->
    <span v-if="showTotal" class="text-sm text-muted-foreground"> 共 {{ total }} 条，第 {{ pageInfo.start }}-{{ pageInfo.end }} 条 </span>
    ...
    >
      <PaginationContent v-slot="{ items }">
        ...
        <template v-for="item in items">
          <PaginationItem v-if="item.type === 'page'" :value="item.value" :is-active="page === item.value" />
