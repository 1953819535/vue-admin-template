...
// 根据布局决定面板展开方向
const sheetSide = computed(() => (appStore.layout === 'sidebar' ? 'left' : 'right'))
...
<template>
  <Sheet v-model:open="sheetOpen">
    ...
    <SheetContent :side="sheetSide" class="w-80 sm:w-96 p-0 flex flex-col h-full">
              ...
              <Tabs v-model="themeTab" class="w-full">
                ...
                <TabsContent value="preset" class="mt-3">
                  ...
                  <div v-if="customPreviewData" class="p-2 rounded-lg border border-primary bg-muted/50 mb-3">
                    ...
                    <div class="h-6 w-full rounded overflow-hidden" :style="{ background: customPreviewData.background }">
                      <div
                        class="h-full flex"
                        :style="{
                  ...
                  <TooltipProvider>
                    <div class="grid grid-cols-3 gap-2">
                      <button
                        ...
                        :class="[
                          appStore.currentTheme === theme.name && !appStore.customTheme ? 'border-primary ring-2 ring-primary/20' : 'border-border',
                        ]"
                        @click="handleSelectTheme(theme.name)"
                      >
                        <div class="mb-1.5 h-6 w-full rounded overflow-hidden" :style="{ background: theme.colors.background }">
                          <div
                            class="h-full flex"
                            :style="{
                              background: `linear-gradient(135deg,
                ...
                <TabsContent value="custom" class="mt-3 space-y-3">
                  <div class="p-2 rounded-lg border bg-muted/30 text-xs text-muted-foreground">
                    从
                    <a href="https://tweakcn.com/community" target="_blank" class="text-primary hover:underline">tweakcn.com</a>
                    复制链接
                  </div>
                  <div class="flex gap-2">
                    <Input v-model="urlInput" placeholder="粘贴 tweakcn 链接..." class="flex-1 h-8 text-xs" />
                    <Button size="sm" class="h-8" :disabled="customTheme.isLoading.value" @click="handleLoadFromUrl">
                  ...
                  <Textarea v-model="customCssInput" placeholder="或直接粘贴 CSS..." class="h-24 text-xs font-mono resize-none" />
                  <div v-if="customTheme.error.value" class="p-2 rounded bg-destructive/10 text-destructive text-xs">
                    {{ customTheme.error.value }}
                  </div>
                  <Button size="sm" class="w-full" @click="handleApplyCustom"> 应用自定义主题 </Button>
                </TabsContent>
              </Tabs>
            </div>
          </div>
