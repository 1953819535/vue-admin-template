    ...
    <PopoverContent class="notification-panel w-96 p-0">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b">
        ...
        <div class="flex gap-2">
          <Button v-if="notificationStore.hasUnread" variant="ghost" size="sm" class="h-7 text-xs" @click="handleMarkAllRead"> 全部已读 </Button>
          <Button
      ...
      <Tabs v-model="activeTab" class="w-full">
        <TabsList class="w-full justify-start rounded-none border-b bg-transparent px-2 h-9">
          <TabsTrigger value="all" class="text-xs data-[state=active]:bg-transparent data-[state=active]:shadow-none px-2">
            全部
          ...
          >
            {{ config.label }}
            <Badge v-if="notificationStore.groupedNotifications[type]?.length > 0" variant="secondary" class="ml-1 h-4 px-1 text-[10px]">
        ...
        <!-- 通知列表 -->
        <TabsContent v-for="tab in ['all', 'system', 'todo', 'notice', 'message']" :key="tab" :value="tab" class="mt-0">
          <ScrollArea class="h-[300px]">
            <!-- 空状态 -->
            <div v-if="displayNotifications.length === 0" class="flex flex-col items-center justify-center py-8 text-muted-foreground">
                ...
                <div class="flex gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="font-medium text-sm truncate" :class="{ 'font-bold': !notification.read }">
                        {{ notification.title }}
                      </span>
                      <span v-if="!notification.read" class="w-2 h-2 rounded-full bg-primary flex-shrink-0" />
                    </div>
      ...
      <div class="border-t px-4 py-2">
        <Button variant="link" size="sm" class="w-full h-7 text-xs text-muted-foreground"> 查看全部通知 </Button>
      </div>
    </PopoverContent>
  </Popover>
</template>
